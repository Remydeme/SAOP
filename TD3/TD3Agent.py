...
class TD3Agent():
    ...
    def __init__(self, env, game, critic_lr=0.0001, actor_lr=0.001, target_action_noise=0.1, action_noise=0.2,  buffer_size=1000000):
    ...
    def updateNetworks(self,steps, batch_size=100, gamma=0.95, noise_clip=0.05):
        ...
        for step in range(steps):
            ...
            next_action = tf.clip_by_value(next_action, -self.env.action_space.high[0], self.env.action_space.high[0])
            ...
            target_value = tf.minimum(t_value, t_value_bis)
            ...
            if step % 2 == 0:
                with tf.GradientTape() as tape:
                    ...
                    c_value, _ = self.critic.get_value(state, action)
                    c_value = - tf.reduce_mean(c_value)
                    gradient = tape.gradient(self.actor.actor.trainable_variables, c_value)

                # apply the gradient
                optimizer = tf.keras.optimizers.Adam(learning_rate=self.actor_lr)
                optimizer.apply_gradients(gradient, self.actor.actor.trainable_variables)
    ...
    def error_and_score_log(self):
        """ Write the error to tensorboard """
        mean_critic_score = np.mean(self.critic_score)
        mean_critic_error = np.mean(self.critic_error)

        tf.summary.scalar("critic score", mean_critic_score)
        tf.summary.scalar("critic error", mean_critic_error)
    ...
    def save_model(self, critic_file_name, actor_file_name):
        ...
        date = self.env_name + datetime.now().strftime("%Y%m%d-%H")

        # model file name
        critic_file_name = critic_file_name + "_" + self.game  + "_" + date
        actor_file_name = actor_file_name + "_"  + self.game + "_" + date

        self.critic.critic.save(critic_file_name)
        self.critic.target_critic.save(critic_file_name + "_target")
